function Layout(children...; title="Therapy × Snapshot")
    Fragment(
        RawHtml("""<link rel="stylesheet" href="/styles.css">"""),
            ...
            Header(:class => "sticky top-0 z-20 border-b border-base-300 bg-base-100/80 backdrop-blur",
                Div(:class => "max-w-5xl mx-auto px-5 h-16 flex items-center justify-between",
                    ...
                    Nav(:class => "flex items-center gap-0.5",
                        A(:href => "/play", :class => "btn btn-ghost btn-sm", "Play"),
                        A(:href => "/guestbook", :class => "btn btn-ghost btn-sm", "Guestbook"),
                        A(:href => "/about", :class => "btn btn-ghost btn-sm hidden sm:inline-flex", "About"),
